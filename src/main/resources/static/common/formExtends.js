...
function GeneralForm(formId) {
	...
	this.submit = function() {
			...
			options.success = function(rInfo) {
				if (rInfo.success) {
				util.show(rInfo.message);
					util.call().refreshDataGrid();
					util.close();
				}else{
				util.alert('提示', rInfo.message, 'error');
				}
			};
			options.beforeSend=function(XMLHttpRequest){disabledForm(formId);WaitDialog.show("正在提交...")};
			options.complete=function(XMLHttpRequest){enabledForm(formId);WaitDialog.hide();};
...
function GeneralForm1(formId, actionName) {
	this.formId = formId;
	this.actionName = actionName;
	this.result = {};
	this.init = function() {
...
var WaitDialog= new function (){
	...
	this.show = function(message){
		...
		var html="<div id=\"loading\" style=\""+style+"\">"+message+"..<img src=\""+applicationContent.basePath+"/style/m/images/waiting.gif\" alt=\"wait...\" /></div>";
		this.div =$(html);
